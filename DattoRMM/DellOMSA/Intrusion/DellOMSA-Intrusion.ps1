...
$Global:AlertHealthy = "No Intrusion Detected | Last Checked $Date" # Define what should be displayed in Datto when monitor is healthy and $Global:AlertMsg is blank.
...
elseif ($output -match "ERROR") {
    ...
    foreach ($intrusion in $RACIntrusions) {
        $Global:DiagMsg += $intrusion.Name + " - " + $intrusion.Intrusion + " | System Status: " + $intrusion.Status
