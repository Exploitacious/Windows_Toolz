Configuration SecurityBaselineConfigurationWS2022
{
	...
	Node localhost
	{
		...
		UserRightsAssignment "CCE-37072-6: Configure 'Allow log on through Remote Desktop Services'"
		{
			...
			Force = $True
			Identity = @('BUILTIN\Administrators', 'Allow-RDP'
			)
	   ...
	   UserRightsAssignment "CCE-37659-0: Configure 'Allow log on locally'"
		{
			...
			Force = $True
			Identity = @('BUILTIN\Administrators', 'BUILTIN\Remote Desktop Users'
			)
...
SecurityBaselineConfigurationWS2022

Start-DscConfiguration -Path ./SecurityBaselineConfigurationWS2022 -Wait -Verbose
