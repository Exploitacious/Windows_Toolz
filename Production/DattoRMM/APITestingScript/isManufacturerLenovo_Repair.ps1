...
if ($System.Manufacturer -match "Lenovo") {
    ...
    if (!$SoftwareResult) {
        ...
        $Global:AlertMsg += "Lenovo System Update is Already Installed!"
    }
}
else {
    $Global:DiagMsg += "Computer reported Manufacturer as " + $System.Manufacturer + " " + $System.Model
    $Global:AlertMsg += "Computer is not a Lenovo!"
...
if ($Global:AlertMsg) {
    ...
    write-DRMMAlert "Successfully Installed System Update $Global:AlertHealthy"
    write-DRMMDiag $Global:DiagMsg
