...
$Global:AlertHealthy = "$Global:totalDisks Physical Disks Fully Operational : Last Checked $Date" # Define what should be displayed in Datto when monitor is healthy and $Global:AlertMsg is blank.
...
try {
    $output = racadm raid get pdisks -o -p "SerialNumber,Status,Name,FailurePredicted,Size" | Out-String
...
elseif ($output -match "ERROR") {
    ...
    # Regex pattern to capture disk information
    [regex]$pattern = '(?ms)Disk\.[^\n]*\n\s*SerialNumber\s*=\s*(\S+)\s*\n\s*Status\s*=\s*(\S+)\s*\n\s*Name\s*=\s*(.+?)\s*\n\s*FailurePredicted\s*=\s*(\S+)\s*\n\s*Size\s*=\s*(.+?)\s*$'

    $RACDisks = $pattern.Matches($output) | ForEach-Object {
        [PSCustomObject]@{
            SerialNumber     = $_.Groups[1].Value.Trim()
            Status           = $_.Groups[2].Value.Trim()
            Name             = $_.Groups[3].Value.Trim()
            FailurePredicted = $_.Groups[4].Value.Trim()
            Size             = $_.Groups[5].Value.Trim()
        }
    }
    ...
    if ($Global:totalDisks -eq 0) {
        $Global:DiagMsg += "No disks found."
    }
    else {
        $Global:DiagMsg += "Total disks: $Global:totalDisks"
    }
}
