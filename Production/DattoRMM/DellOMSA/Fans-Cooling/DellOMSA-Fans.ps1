...
$Global:AlertHealthy = "Fans Fully Operational | Last Checked $Date" # Define what should be displayed in Datto when monitor is healthy and $Global:AlertMsg is blank.
...
# Initialize output variable and Fan count
$output = ""
$Global:totalFans = 0
...
elseif ($output -match "ERROR") {
    ...
    #Fan Count
    $Global:totalFans = $RACFans.Count

    foreach ($fan in $RACFans) {
        $Global:DiagMsg += $fan.Name + " " + $fan.Status + " | Spinning @ " + $fan.Reading
    ...
    if ($Global:totalFans -eq 0) {
        $Global:DiagMsg += "No fans found."
    }
    else {
        $Global:DiagMsg += "Total fans: $Global:totalFans"
    }
}
#END
...
if ($Global:AlertMsg) {
    ...
    write-DRMMAlert "$Global:totalFans $Global:AlertHealthy"
    write-DRMMDiag $Global:DiagMsg
