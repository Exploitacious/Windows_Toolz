...
elseif ($output -match "ERROR") {
    ...
    $Global:totalDIMMs = $RACMemoryModules.Count
    ...
    if ($Global:totalDIMMs -eq 0) {
        $Global:DiagMsg += "No Memory DIMMs found!"
        $Global:AlertMsg += "Error Occurred - Scrutinize Diagnostic Log"
    }
    else {
        $Global:DiagMsg += "Total Memory DIMMs in use: $Global:totalDIMMs"
    }
}
...
if ($Global:AlertMsg) {
    ...
    write-DRMMAlert "$Global:totalDIMMs $Global:AlertHealthy"
    write-DRMMDiag $Global:DiagMsg
