...
$env:TempAlertThresh = 90 #Testing # $TempAlertThresh = 99 # Set the variable in Datto RMM
...
$output = ""
$Global:sensorsCount = 0
$Global:totalTemperature = 0
$Global:averageTemperature = 0
...
elseif ($output -match "ERROR") {
    ...
    foreach ($sensor in $RACTemperatureSensors) {
        $Global:SensorsCount++
        $Global:DiagMsg += "$($sensor.Name) Reading $($sensor.Reading) : $($sensor.Status) "
        ...
        $temperature = [float]$sensor.Reading.TrimEnd('C')
        $Global:totalTemperature += $temperature
    }
    $Global:DiagMsg += "Temperature Sensors Detected: $Global:SensorsCount"
    
    if ($Global:SensorsCount -gt 0) {
        $Global:averageTemperature = $Global:totalTemperature / $Global:SensorsCount
        $Global:DiagMsg += "Average Temperature Reading: $Global:averageTemperature C"
        
        if ($Global:averageTemperature -gt $env:TempAlertThresh) {
            $Global:AlertMsg += "Average Temperature is Above $env:TempAlertThresh C | Current Average Reading: $Global:averageTemperature C"
            $Global:DiagMsg += "Average Temperature is Above Specified Threshhold of $env:TempAlertThresh C"
        }
    }
    else {
        $Global:DiagMsg += "No valid temperature readings found."
        $Global:DiagMsg += "No Temp Sensors found."
        $Global:AlertMsg += " Temperature Sensor Status is Unknown"
    }    
}
...
if ($Global:AlertMsg) {
    ...
    write-DRMMAlert "Average Temperature: $Global:averageTemperature C $Global:AlertHealthy"
    write-DRMMDiag $Global:DiagMsg
