...
# 1A: Check Physical Disk Health (S.M.A.R.T. & WMIC)
$Global:DiagMsg += "Checking physical disk health (WMIC & S.M.A.R.T.)..."
try {
    $physicalDisks = Get-PhysicalDisk -ErrorAction Stop
    $wmicDiskStatus = wmic diskdrive get model, status
    
    $diskChecksList = New-Object System.Collections.Generic.List[System.Object]

    foreach ($disk in $physicalDisks) {
        $wmicStatusLine = $wmicDiskStatus | Where-Object { $_ -match [regex]::Escape($disk.FriendlyName) } | Select-Object -First 1
        $wmicStatus = if ($wmicStatusLine) { ($wmicStatusLine.Trim() -split '\s+')[-1] } else { 'N/A' }
