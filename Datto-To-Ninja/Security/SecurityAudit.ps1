...
if (!([System.Diagnostics.EventLog]::SourceExists("Datto RMM Security Audit"))) {
    New-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit'
}

# preliminary pabulum
write-host "Datto RMM Security Audit: build $varBuildString" # by seagull, august 2020, january 2023
write-host `r
...
if ($varKernel -lt 7601) {
    write-host "- ALERT: This Component only runs on devices running Windows 7 SP1 and higher."
    write-host "  (Windows 7 and Server 2008 extended support was cut off in January 2023,"
    write-host "  with ESUs available strictly for Azure-based 2008 installations until 2024.)"
    write-host "  Updating, preferably decommissioning, of this device is strongly advised."
    write-host `r
    write-host "  https://learn.microsoft.com/en-us/lifecycle/products/windows-7"
    write-host "  https://learn.microsoft.com/en-us/lifecycle/products/windows-server-2008"
    $varAlerts++
    Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Error -EventID 59101 -Message "Datto RMM Security Audit Alert: This OS is not supported by Microsoft and will not be receiving Security Updates.`rIt is crucial that this device is upgraded or decommissioned as soon as possible.`rThe audit cannot proceed."
    exit
}

if ($varKernel -eq 7601) {
    if ($varDomainRole -le 1) {
        # windows 7
        write-host "- ALERT: Support for Windows 7 was discontinued on the 14th of January 2020."
        write-host "  Extended support was discontinued on the 10th of January 2023."
        write-host "  There is no longer any support in any form for this operating system."
        write-host "  https://learn.microsoft.com/en-us/lifecycle/products/windows-7"
        Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Error -EventID 59103 -Message "Datto RMM Security Audit Alert: This OS is not supported by Microsoft and will not be receiving Security Updates.`rIt is crucial that this device is upgraded or decommissioned as soon as possible."
    }
    else {
        #windows server 2008 r2
        write-host "- ALERT: Support for Windows Server 2008 R2 was discontinued on the 10th of January 2023."
        write-host "  This can be mitigated for another year by moving to Azure; if this device has already been"
        write-host "  migrated to Azure, this message can be disregarded until January 2024."
        write-host "  https://learn.microsoft.com/en-us/lifecycle/products/windows-server-2008-r2"
        Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Error -EventID 59103 -Message "Datto RMM Security Audit Alert: This OS is not supported by Microsoft and will not be receiving Security Updates.`rIt is crucial that this device is upgraded or decommissioned as soon as possible.`rIf this device is receiving Extended Security Updates (ESUs) from Microsoft this message can be disregarded."
    }
    $varAlerts++
    write-host `r
}

if ($varKernel -eq 9200 -or $varKernel -eq 9600) {
    if ($varDomainRole -le 1) {
        #windows 8/8.1
        write-host "- ALERT: Windows 8.0 and 8.1 have been discontinued by Microsoft."
        write-host "  Unlike with Windows 7, there are no ESUs or Azure offerings; the OS is fully EOL."
        write-host "  Please update this device to Windows 10/11."
        write-host "  https://learn.microsoft.com/en-us/lifecycle/products/windows-81"
        $varAlerts++
        write-host `r
        Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Error -EventID 59103 -Message "Datto RMM Security Audit Alert: This OS is not supported by Microsoft and will not be receiving Security Updates.`rIt is crucial that this device is upgraded or decommissioned as soon as possible."
    }
        ...
        if ($([int][double]::Parse((Get-Date -UFormat %s))) -gt 1696896000) {
            #on or after 10 oct 2023 00:00:00
            write-host "- ALERT: Windows Server 2012 and 2012 R2 was discontinued by Microsoft on the 10th Oct 2023."
            write-host "  Extended Support Updates (ESUs) are available until 2026; if this device is making use of"
            write-host "  such updates, this message can be disregarded."
            write-host "  https://learn.microsoft.com/en-us/lifecycle/products/windows-server-2012"
            write-host "  https://learn.microsoft.com/en-us/lifecycle/products/windows-server-2012-r2"
            $varAlerts++
            write-host `r
            Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Error -EventID 59103 -Message "Datto RMM Security Audit Alert: This OS is not supported by Microsoft and will not be receiving Security Updates.`rIt is crucial that this device is upgraded or decommissioned as soon as possible.`rIf this device is receiving Extended Security Updates (ESUs) from Microsoft this message can be disregarded."
        }
...
If ( -not $localAccountExists ) {
    ...
    if ((Get-WmiObject -Class Win32_UserAccount -Filter "LocalAccount='$true' AND SID LIKE '%-501'").disabled) {
        ...
        $varAlerts++
        Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Warning -EventID 59104 -Message "Datto RMM Security Audit Warning: The Guest account is enabled on this device.`rThis unprotected user account can be used as a vantage point by malware and should be disabled."
    }
    ...
    if ((Get-WmiObject -Class Win32_UserAccount -Filter "LocalAccount='$true' AND SID LIKE '%-500'").disabled) {
        ...
        $varAlerts++
        Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Warning -EventID 59105 -Message "Datto RMM Security Audit Warning: The local Administrator account is enabled on this device.`rThis can be used as a vantage point by malware and should be disabled."
    }
    ...
    if ($arrLocalAdmins) {
        ...
        foreach ($iteration in $arrLocalAdmins) {
            ...
            if ((Get-WmiObject -Class Win32_ComputerSystem).PartOfDomain) {
                Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Warning -EventID 59106 -Message "Datto RMM Security Audit Warning: The local user `"$iteration`" is listed as an Administrator.`rLocal users should not have device-level administrative privileges; devices should be governed by the network administrator."
            }
            else {
                Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Warning -EventID 59106 -Message "Datto RMM Security Audit Warning: The local user `"$iteration`" is listed as an Administrator.`rLocal users should not have device-level administrative privileges; the device should be part of, and governed by, a domain."
            }
        }
    }
    else {
...
try {
    ...
    $varAlerts++
    Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Warning -EventID 59107 -Message "Datto RMM Security Audit Warning: Account password for user `"$varDefaultUser`" stored in plaintext in Registry.`rThe user appears to have configured their device to log into their user account automatically via the Registry.`rTheir password is stored in plaintext at HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon under value `"DefaultPassword`" and should be removed ASAP."
    ...
    if ($varDefaultPassLength -le 7) {
        ...
        Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Warning -EventID 59108 -Message "Datto RMM Security Audit Warning: Since the password for user `"$varDefaultUser`" is stored in plaintext in the Registry, it's been analysed for length.`rThe password is fewer than 8 characters in length. Implement a stronger password or stronger password policy settings."
    }
    # -- strength
    if ($varDefaultPass -match 'password' -or $varDefaultPass -match 'p4ssw0rd' -or $varDefaultPass -match '12345' -or $varDefaultPass -match 'qwerty' -or $varDefaultPass -match 'letmein') {
        ...
        Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Error -EventID 59109 -Message "Datto RMM Security Audit Alert: Since the password for user `"$varDefaultUser`" is stored in plaintext in the Registry, it's been analysed for security.`rThe password is one of many very well-known common password strings. Implement a more unique password or stronger password policy settings."
    }
}
...
try {
    ...
    if ($varNullSession -ne 1) {
        ...
        $varAlerts++
        Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Warning -EventID 59110 -Message "Datto RMM Security Audit Warning: Access to anonymous shares is permitted and should be disabled.`rThe setting is stored in the Registry at HKLM:\SYSTEM\CurrentControlSet\Services\LanManServer\Parameters under value `"RestrictNullSessAccess`"."
    }
    else {
...
if ($?) {
    ...
    $varAlerts++
    Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Warning -EventID 59111 -Message "Datto RMM Security Audit Warning: Telnet Server is running and should be replaced by a more secure alternative."
}
else {
    write-host "+ Telnet Server is not installed."
}

# is SMBv1 permitted? :: SMBv1 "client" instructions removed; this new command encapsulates everything
if ($varKernel -match '^76') {
    try {
        if ((Get-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Services\LanmanServer\Parameters').SMB1 -ne 0) {
            write-host "- ALERT: Device SMBv1 support has not been disabled."
            write-host "  This is a huge security risk. This protocol is actively exploited by malware."
            $varAlerts++
            Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Warning -EventID 59112 -Message "Datto RMM Security Audit Warning: Device uses the vulnerable and actively-exploited SMBv1 protocol.`rMicrosoft advisory: https://blogs.technet.microsoft.com/filecab/2016/09/16/stop-using-smb1/"
        }
        else {
            write-host "+ Device is not configured with SMBv1 support."
        }
    }
    catch {
        write-host "+ Device is not configured with SMBv1 support."
    ...
    if ((Get-WindowsOptionalFeature -Online -FeatureName SMB1Protocol).State.value__ -ne 0) {
        if ((Get-SmbServerConfiguration).EnableSMB1Protocol) {
            write-host "- ALERT: Device is configured with SMBv1 support."
            write-host "  This is a huge security risk. This protocol is actively exploited by malware."
            $varAlerts++
            Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Warning -EventID 59112 -Message "Datto RMM Security Audit Warning: Device serves using the vulnerable and actively-exploited SMBv1 protocol.`rMicrosoft advisory: https://blogs.technet.microsoft.com/filecab/2016/09/16/stop-using-smb1/`rDisable (PS): Disable-WindowsOptionalFeature -Online -FeatureName SMB1Protocol"
        }
        else {
            write-host "+ Device supports SMBv1, but it has been disabled."
        }
    }
    else {
        write-host "+ Device is not configured with SMBv1 support."
...
if (((Get-WmiObject win32_service  -Filter "name like '%mpssvc%'").state) -match 'Running') {
    ...
    $varAlerts++
    Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Warning -EventID 59119 -Message "Datto RMM Security Audit Warning: Windows Firewall is not running.`rIf this was unintentional, please re-enable Windows Firewall.`rIf this was intentional, please ensure the replacement solution is operational and configured."
}
...
try {
    ...
    if ($varSMBCheck -ne 1) {
        ...
        $varAlerts++
        Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Warning -EventID 59116 -Message "Datto RMM Security Audit Warning: Windows Firewall is disabled for Private networks.`rIf this was unintentional, please revert the setting.`rIf this was intentional, please ensure the replacement solution is operational and configured."
    }
    else {
...
try {
    ...
    if ($varSMBCheck -ne 1) {
        ...
        $varAlerts++
        Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Warning -EventID 59117 -Message "Datto RMM Security Audit Warning: Windows Firewall is disabled for Public networks.`rIf this was unintentional, please revert the setting.`rIf this was intentional, please ensure the replacement solution is operational and configured."
    }
    else {
...
if ((Get-WmiObject -Class Win32_ComputerSystem).PartOfDomain) {
    try {
        ...
        if ($varSMBCheck -ne 1) {
            ...
            $varAlerts++
            Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Warning -EventID 59118 -Message "Datto RMM Security Audit Warning: Windows Firewall is disabled for Domains.`rIf this was unintentional, please revert the setting.`rIf this was intentional, please ensure the replacement solution is operational and configured."
        }
        else {
...
if ($varKernel -ge 9200) {
    try {
        ...
        if ($varSecureBoot) {
            ...
            $varAlerts++
            Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Warning -EventID 59114 -Message "Datto RMM Security Audit Warning: UEFI Secure Boot is supported on this device but has not been enabled.`rThis may have been configured deliberately to facilitate installation of other Operating Systems that do not have a Microsoft Secure Boot shim available; however, the setting still leaves a device vulnerable and should be changed."
        }
    }
...
if ($varKernel -ge 16299) {
    ...
    if ($varExploitFlaws) {
        ...
        $varWarnings++
        Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Warning -EventID 59115 -Message "Datto RMM Security Audit Warning: Windows 10 Exploit Protection settings have been altered from the default.`rThis is generally done deliberately by the end-user or administrator in order to mitigate against a specific compatibility or performance issue.`rRegardless, it is bad practice to deviate from Microsoft's standards. Please scrutinise the mitigation steps below and ensure you have a strong justification for dismissing each.`r$varExploitFlaws"
    }
    else {
...
try {
    Get-ChildItem -Recurse 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\Safer\codeidentifiers\0\Paths' -ErrorAction stop | % {
    ...
    write-host "  Please consider blocking execution of CPL, SCR, and VBS in SecPol.msc."
    $varAlerts++
    Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Error -EventID 59120 -Message "Datto RMM Security Audit Alert: The Windows Security Policy is not configured to block files with dangerous extensions from executing.`rThese file types are: CPL, SCR, VBS."
}

#security policy II: parse the data
if ($varSecPols) {
...
if ($varFileRisks) {
    write-host "- ALERT: The Security Policy does not prohibit execution of problematic file types."
    write-host "  These file types are: $varFileRisks"
    $varAlerts++
    Write-EventLog -LogName 'Application' -Source 'Datto RMM Security Audit' -EntryType Error -EventID 59120 -Message "Datto RMM Security Audit Alert: The Windows Security Policy is not configured to block files with dangerous extensions from executing.`rThese file types are: $varFileRisks`."
}
