<#fireEye red team vuln scanner :: build 24/seagull (Datto RMM) :: user variables: usrScanScope
...
if ($env:CS_CC_HOST) {
    write-host "====================================================="
    write-host ": Alert on detections using a Datto RMM File Size Monitor."
    write-host "  File size set at 0MB. File(s) named:"
    write-host "  $PWD\detections.txt"
}
...
<#
all-yara.yar	180.9 KB
yara-COPYING.txt	1.5 KB
yara32.exe	1.4 MB
yara64.exe	2.1 MB
#>