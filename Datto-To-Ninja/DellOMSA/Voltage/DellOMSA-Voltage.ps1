...
$Global:AlertHealthy = "Voltage Sensors Detected | Last Checked $Date" # Define what should be displayed in Datto when monitor is healthy and $Global:AlertMsg is blank.
...
elseif ($output -match "ERROR") {
    ...
    $Global:DiagMsg += "Total Voltage Sensors Detected: $Global:sensorsCount"
}
...
if ($Global:AlertMsg) {
    ...
    write-DRMMAlert "$Global:sensorsCount $Global:AlertHealthy"
    write-DRMMDiag $Global:DiagMsg
