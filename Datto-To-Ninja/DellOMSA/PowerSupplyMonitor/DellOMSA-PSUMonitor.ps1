...
$Global:AlertHealthy = "PSUs Operational | Last Checked $Date" # Define what should be displayed in Datto when monitor is healthy and $Global:AlertMsg is blank.
...
elseif ($output -match "ERROR") {
    ...
    $Global:DiagMsg += "$Global:PSUCount Power Supplie(s) Detected"
    $Global:DiagMsg += "Check Voltage Monitors for more power details"
}
...
if ($Global:AlertMsg) {
    ...
    write-DRMMAlert "$Global:PSUCount $Global:AlertHealthy"
    write-DRMMDiag $Global:DiagMsg
