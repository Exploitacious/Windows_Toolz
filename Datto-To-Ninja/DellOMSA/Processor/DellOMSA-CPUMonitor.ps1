...
$Global:AlertHealthy = "CPUs Detected | Last Checked $Date" # Define what should be displayed in Datto when monitor is healthy and $Global:AlertMsg is blank.
...
elseif ($output -match "ERROR") {
    ...
    $Global:DiagMsg += "$Global:CPUCount CPUs Detected"
}
...
if ($Global:AlertMsg) {
    ...
    write-DRMMAlert "$Global:CPUCount $Global:AlertHealthy"
    write-DRMMDiag $Global:DiagMsg
