function Initialize-PSGallery {
    ...
    .NOTES
    Version:        1.0
    Author:         Chris Taylor / Kelvin Tegelaar
    Creation Date:  16/08/2021
    Purpose/Change: Datto conversion
    #>
    [cmdletbinding()]
    Param()
    ...
    function Register-PSGallery {
        if ($Host.Version.Major -gt 4) { Register-PSRepository -Default }
            ...
            try {
                ...
                write-host "Didn't register the PSGallery. Try running the script again"
            }
        }
    }
    ...
    try {
        ...
        $TempPath = "PowerShellGetModules.zip"
            ...
            $env:PSModulePath -split ';' | ForEach-Object {
                ...
                if ((Test-Path $Path)) {
                    ...
                    try {
                        ...
                        write-host "Not able to import all modules, but that's okay. The device might not be ready yet."
                    }
                }
            }
